...
namespace OlapFormsFramework.Windows.Forms.Grid.Scatter
{
	public class OlapScatter : GMapControl, IActionListener
	{
		...
		private void HintVisibilityUpdate(int aID)
		{
			if ((_Data.ItemSelectionGet(aID) & SIST.sistAsItem) == 0)
			{
				if (_Data.ItemHintGet(aID) != null && _Data.ItemHintGet(aID).Visible)
					HintHide(aID, _Data.CurrentPage);
		...
		private readonly Dictionary<OSDI, OlapMapCircleMarker> _markerDataItem = new Dictionary<OlapScatterDataItem, OlapMapCircleMarker>();
		...
		{
			Graphics graphics = CreateGraphics();//_bufferedPage.BufferedGraphics;
		...
		private void ItemsDraw(Graphics aGraphics)
		{
			 ...
			 */
			#endregion
			for (int j = 0; j < _Data.ItemsCount + _groupHighlightIndixes.Count; ++j)
			{
				...
				else    //відмальовка відбувається для позиції що знаходитьсь між двох сторінок
				{
					...
					//var cr = (int)(p.Color.R + (n.Color.R - p.Color.R) * coefficient);
					//var cg = (int)(p.Color.G + (n.Color.G - p.Color.G) * coefficient);
					//var cb = (int)(p.Color.B + (n.Color.B - p.Color.B) * coefficient);
					//c = Color.FromArgb(a, cr, cg, 
				}
#if DEBUG
				++drawedItems;
#endif
				...
				if (aGraphics.ClipBounds.Contains(x, y) && Math.Abs(s) <= 1000000f)
				{
					...
					//if (interpolated)
					//{
					//	var diagPoint = (float)Math.Sqrt(1f / 2) * radius;
					//	Pen pen = null;
					//	try
					//	{
