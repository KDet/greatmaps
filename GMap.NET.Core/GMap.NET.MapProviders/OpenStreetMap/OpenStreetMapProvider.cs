...
namespace GMap.NET.MapProviders
{
	public abstract class OpenStreetMapProviderBase : GMapProvider, RoutingProvider, GeocodingProvider
	{
		...
		{
			var points = GetRoutePoints(MakeRoutingUrl(start, end, walkingMode ? TravelTypeFoot : TravelTypeMotorCar));
		...
		private List<PointLatLng> GetRoutePoints(string url)
		{
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(route))
				{
					...
					var coordNode = xmldoc.SelectSingleNode("/sm:kml/sm:Document/sm:Folder/sm:Placemark/sm:LineString/sm:coordinates",
						xmlnsManager);
					...
					if (coordinates.Length > 0)
					{
						...
						foreach (var coordinate in coordinates)
						{
							if (coordinate != string.Empty)
							{
								...
								if (XY.Length == 2)
								{
									var lat = double.Parse(XY[1], CultureInfo.InvariantCulture);
									var lng = double.Parse(XY[0], CultureInfo.InvariantCulture);
		...
		{
			var street = String.Join(" ", placemark.HouseNo, placemark.ThoroughfareName).Trim();
						...
						doc.LoadXml(geo);
						{
                           ...
                           if(nn != null && !string.IsNullOrEmpty(nn.Value))
                           {
                              rank = int.Parse(nn.Value, NumberStyles.Integer, CultureInfo.InvariantCulture);
									...
									if (nn != null)
									{
										var lat = double.Parse(nn.Value, CultureInfo.InvariantCulture);

										nn = n.Attributes["lon"];
										if (nn != null)
										{
											var lng = double.Parse(nn.Value, CultureInfo.InvariantCulture);
