	...
	public abstract class GoogleMapProviderBase : GMapProvider, RoutingProvider, GeocodingProvider, DirectionsProvider
	{
		...
		public override void OnInitialized()
		{
			if (!init && TryCorrectVersion)
			{
				...
				try
				{
					var html = GMaps.Instance.UseUrlCache
					...
					if (!string.IsNullOrEmpty(html))
					{
						...
						if (mat.Success)
						{
							...
							if (count > 0)
							{
								...
								var old = GMapProviders.GoogleMap.Version;

								GMapProviders.GoogleMap.Version = ver;
								GMapProviders.GoogleChinaMap.Version = ver;

								var verh = string.Format("h@{0}", gc[1].Value);
								var oldh = GMapProviders.GoogleHybridMap.Version;
						...
						if (mat.Success)
						{
							...
							{
								var ver = gc[1].Value;
								var old = GMapProviders.GoogleSatelliteMap.Version;
						...
						if (mat.Success)
						{
							...
							if (count > 1)
							{
								...
								var old = GMapProviders.GoogleTerrainMap.Version;
		...
		internal void GetSecureWords(GPoint pos, out string sec1, out string sec2)
		{
			...
			var seclen = (int) ((pos.X*3) + pos.Y)%8;
		...
		public MapRoute GetRoute(PointLatLng start, PointLatLng end, bool avoidHighways, bool walkingMode, int Zoom)
		{
			...
			int zoomFactor;
			MapRoute ret = null;
			var points = GetRoutePoints(MakeRouteUrl(start, end, LanguageStr, avoidHighways, walkingMode), Zoom, out tooltip,
				out numLevels, out zoomFactor);
			if (points != null)
			{
		...
		{
			var opt = walkingMode ? WalkingStr : (avoidHighways ? RouteWithoutHighwaysStr : RouteStr);
			return string.Format(CultureInfo.InvariantCulture, RouteUrlFormatPointLatLng, language, opt, start.Lat, start.Lng,
				end.Lat, end.Lng, Server);
		}
			...
			out int zoomFactor)
		{
			...
			try
			{
				var route = GMaps.Instance.UseRouteCache ? Cache.Instance.GetContent(url, CacheType.RouteCache) : string.Empty;
				...
				if (!string.IsNullOrEmpty(route))
				{
					...
					var tooltipEnd = 0;
					{
						var x = route.IndexOf("tooltipHtml:") + 13;
						if (x >= 13)
						{
					...
					var pointsEnd = 0;
					{
						var x = route.IndexOf("points:", tooltipEnd >= 0 ? tooltipEnd : 0) + 8;
						if (x >= 8)
						{
					...
					var levelsEnd = 0;
					{
						var x = route.IndexOf("levels:", pointsEnd >= 0 ? pointsEnd : 0) + 8;
						if (x >= 8)
						{
					...
					var numLevelsEnd = 0;
					{
						var x = route.IndexOf("numLevels:", levelsEnd >= 0 ? levelsEnd : 0) + 10;
						if (x >= 10)
						{
							numLevelsEnd = route.IndexOf(",", x);
							if (numLevelsEnd > 0)
							{
								var l = numLevelsEnd - x;
								if (l > 0)
								{
					...
					{
						var x = route.IndexOf("zoomFactor:", numLevelsEnd >= 0 ? numLevelsEnd : 0) + 11;
						if (x >= 11)
						{
							var end = route.IndexOf("}", x);
							if (end > 0)
							{
								var l = end - x;
								if (l > 0)
					...
					if (points != null && numLevel > 0 && !string.IsNullOrEmpty(levels))
					{
						...
						//
						var allZlevels = "TSRPONMLKJIHGFEDCBA@?";
						...
						var pLevels = allZlevels.Substring(allZlevels.Length - numLevel);

						// remove useless points at zoom
						{
							var removedPoints = new List<PointLatLng>();

							for (var i = 0; i < levels.Length; i++)
							{
								var zi = pLevels.IndexOf(levels[i]);
								if (zi > 0)
								{
									if (zi*numLevel > zoom)
									{
		...
		private static readonly string RouteUrlFormatPointLatLng =
			"http://maps.{6}/maps?f=q&output=dragdir&doflg=p&hl={0}{1}&q=&saddr=@{2},{3}&daddr=@{4},{5}";

		private static readonly string RouteUrlFormatStr =
		...
		{
			return string.Format(CultureInfo.InvariantCulture, GeocoderUrlFormat, ServerAPIs,
				Uri.EscapeDataString(keywords).Replace(' ', '+'), language);
		}
		...
		private GeoCoderStatusCode GetLatLngFromGeocoderUrl(string url, out List<PointLatLng> pointList)
		{
			...
			try
			{
				var geo = GMaps.Instance.UseGeocoderCache ? Cache.Instance.GetContent(url, CacheType.GeocoderCache) : string.Empty;
							...
							else
							{
								...
								if (l != null)
								{
									foreach (XmlNode n in l)
									{
										...
										if (nn != null)
										{
											var lat = double.Parse(nn.InnerText, CultureInfo.InvariantCulture);

											nn = n.SelectSingleNode("geometry/location/lng");
											if (nn != null)
											{
												var lng = double.Parse(nn.InnerText, CultureInfo.InvariantCulture);
		...
		private GeoCoderStatusCode GetPlacemarkFromReverseGeocoderUrl(string url, out List<Placemark> placemarkList)
		{
			...
			try
			{
				var reverse = GMaps.Instance.UsePlacemarkCache
		...
		private static readonly string ReverseGeocoderUrlFormat =
			"http://maps.{0}/maps/api/geocode/xml?latlng={1},{2}&language={3}&sensor=false";

		private static readonly string GeocoderUrlFormat =
			"http://maps.{0}/maps/api/geocode/xml?address={1}&language={2}&sensor=false";
			...
			bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
		{
			return
				GetDirectionsUrl(
					MakeDirectionsUrl(start, end, LanguageStr, avoidHighways, avoidTolls, walkingMode, sensor, metric), out direction);
		}

		public DirectionsStatusCode GetDirections(out GDirections direction, string start, string end, bool avoidHighways,
		...
		/// <param name="metric"></param>
		/// <returns></returns>
		public IEnumerable<GDirections> GetDirections(out DirectionsStatusCode status, PointLatLng start, PointLatLng end,
			bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
		{
			// TODO: add alternative directions
			...
			bool sensor, bool metric)
		{
			return
				GetDirectionsUrl(
					MakeDirectionsUrl(start, wayPoints, end, LanguageStr, avoidHighways, avoidTolls, walkingMode, sensor, metric),
					out direction);
		}

		public DirectionsStatusCode GetDirections(out GDirections direction, string start, IEnumerable<string> wayPoints,
		...
		private string MakeDirectionsUrl(PointLatLng start, PointLatLng end, string language, bool avoidHighways,
			bool avoidTolls, bool walkingMode, bool sensor, bool metric)
		{
			var av = (avoidHighways ? "&avoid=highways" : string.Empty) + (avoidTolls ? "&avoid=tolls" : string.Empty); // 6
			var mt = "&units=" + (metric ? "metric" : "imperial"); // 7
			var wk = "&mode=" + (walkingMode ? "walking" : "driving"); // 8

			return string.Format(CultureInfo.InvariantCulture, DirectionUrlFormatPoint, start.Lat, start.Lng, end.Lat, end.Lng,
				sensor.ToString().ToLower(), language, av, mt, wk, ServerAPIs);
		}
		...
		{
			var av = (avoidHighways ? "&avoid=highways" : string.Empty) + (avoidTolls ? "&avoid=tolls" : string.Empty); // 4
			var mt = "&units=" + (metric ? "metric" : "imperial"); // 5
			var wk = "&mode=" + (walkingMode ? "walking" : "driving"); // 6

			return string.Format(DirectionUrlFormatStr, start.Replace(' ', '+'), end.Replace(' ', '+'),
				sensor.ToString().ToLower(), language, av, mt, wk, ServerAPIs);
		}

		private string MakeDirectionsUrl(PointLatLng start, IEnumerable<PointLatLng> wayPoints, PointLatLng end,
			string language, bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
		{
			...
			return string.Format(CultureInfo.InvariantCulture, DirectionUrlFormatWaypoint, start.Lat, start.Lng, wpLatLng,
				sensor.ToString().ToLower(), language, av, mt, wk, ServerAPIs, end.Lat, end.Lng);
		}

		private string MakeDirectionsUrl(string start, IEnumerable<string> wayPoints, string end, string language,
			bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
		{
			var av = (avoidHighways ? "&avoid=highways" : string.Empty) + (avoidTolls ? "&avoid=tolls" : string.Empty); // 6
			var mt = "&units=" + (metric ? "metric" : "imperial"); // 7
			...
			return string.Format(CultureInfo.InvariantCulture, DirectionUrlFormatWaypointStr, start.Replace(' ', '+'), wpLatLng,
				sensor.ToString().ToLower(), language, av, mt, wk, ServerAPIs, end.Replace(' ', '+'));
		}
		...
		private DirectionsStatusCode GetDirectionsUrl(string url, out GDirections direction)
		{
			...
			try
			{
				var kml = GMaps.Instance.UseDirectionsCache
					? Cache.Instance.GetContent(url, CacheType.DirectionsCache)
					: string.Empty;
		...
		private static readonly string DirectionUrlFormatStr =
			"http://maps.{7}/maps/api/directions/xml?origin={0}&destination={1}&sensor={2}&language={3}{4}{5}{6}";

		private static readonly string DirectionUrlFormatPoint =
			"http://maps.{9}/maps/api/directions/xml?origin={0},{1}&destination={2},{3}&sensor={4}&language={5}{6}{7}{8}";

		private static readonly string DirectionUrlFormatWaypoint =
			"http://maps.{8}/maps/api/directions/xml?origin={0},{1}&waypoints={2}&destination={9},{10}&sensor={3}&language={4}{5}{6}{7}";

		private static readonly string DirectionUrlFormatWaypointStr =
			"http://maps.{7}/maps/api/directions/xml?origin={0}&waypoints={1}&destination={8}&sensor={2}&language={3}{4}{5}{6}";
