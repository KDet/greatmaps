...
namespace GMap.NET.MapProviders
{
	public abstract class YahooMapProviderBase : GMapProvider, GeocodingProvider
	{
		...
		{
			return string.Format(CultureInfo.InvariantCulture, GeocoderUrlFormat, keywords.Replace(' ', '+'), AppId,
		...
		{
			return string.Format(CultureInfo.InvariantCulture, ReverseGeocoderUrlFormat, pt.Lat, pt.Lng, AppId,
				!string.IsNullOrEmpty(LanguageStr) ? "&locale=" + LanguageStr : "");
		}
		...
		private GeoCoderStatusCode GetPlacemarksFromReverseGeocoderUrl(string url, out List<Placemark> placemarkList)
		{
			...
			try
			{
				var geo = GMaps.Instance.UsePlacemarkCache ? Cache.Instance.GetContent(url, CacheType.PlacemarkCache) : string.Empty;
		...
		private static readonly string GeocoderDetailedUrlFormat =
	...
	public class YahooMapProvider : YahooMapProviderBase
	{
		...
		private readonly Guid _id = new Guid("65DB032C-6869-49B0-A7FC-3AE41A26AF4D");
		...
		private readonly string _name = "YahooMap";
		...
		private string MakeTileImageUrl(GPoint pos, int zoom, string language)
		{
			...
			return string.Format(UrlFormat, GetServerNum(pos, 2) + 1, Version, zoom, pos.X, pos.Y, language, _rnd1, _rnd2);
		}

		private readonly string _rnd1 = Guid.NewGuid().ToString("N").Substring(0, 28);
		private readonly string _rnd2 = Guid.NewGuid().ToString("N").Substring(0, 20);
