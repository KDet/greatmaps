...
namespace GMap.NET.MapProviders
{
	public abstract class CloudMadeMapProviderBase : GMapProvider, RoutingProvider, DirectionsProvider
	{
		...
		{
			var points =
				GetRoutePoints(MakeRoutingUrl(start, end, walkingMode ? TravelTypeFoot : TravelTypeMotorCar, LanguageStr, "km"));
		...
		private string MakeRoutingUrl(PointLatLng start, PointLatLng end, string travelType, string language, string units)
		{
			...
			return string.Format(CultureInfo.InvariantCulture, UrlFormat, Key, Version, start.Lat, start.Lng, end.Lat, end.Lng,
				travelType, language, units);
		}
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(route))
				{
					...
					if (wpts != null && wpts.Count > 0)
					{
						...
						{
							var lat = double.Parse(w.Attributes["lat"].InnerText, CultureInfo.InvariantCulture);
							var lng = double.Parse(w.Attributes["lon"].InnerText, CultureInfo.InvariantCulture);
			...
			bool avoidHighways, bool avoidTolls, bool walkingMode, bool sensor, bool metric)
		{
			return
				GetDirectionsUrl(
					MakeRoutingUrl(start, end, walkingMode ? TravelTypeFoot : TravelTypeMotorCar, LanguageStr, metric ? "km" : "miles"),
					out direction);
		}
		...
		private DirectionsStatusCode GetDirectionsUrl(string url, out GDirections direction)
		{
			...
			try
			{
				var route = GMaps.Instance.UseRouteCache ? Cache.Instance.GetContent(url, CacheType.DirectionsCache) : string.Empty;
				if (string.IsNullOrEmpty(route))
				{
					route = GetContentUsingHttp(url);
					if (!string.IsNullOrEmpty(route))
	...
	public class CloudMadeMapProvider : CloudMadeMapProviderBase
	{
		...
		{
			return string.Format(UrlFormat, ServerLetters[GetServerNum(pos, 3)], Key, StyleID,
				(DoubleResolution ? DoubleResolutionString : string.Empty), zoom, pos.X, pos.Y);
		}
