	...
	public abstract class BingMapProviderBase : GMapProvider, RoutingProvider, GeocodingProvider
	{
		...
		public override void OnInitialized()
		{
			if (!init)
			{
				try
				{
					...
					{
						var keyResponse = GMaps.Instance.UseUrlCache
							? Cache.Instance.GetContent("BingLoggingServiceV1" + key, CacheType.UrlCache, TimeSpan.FromHours(8))
							: string.Empty;
					...
					if (TryCorrectVersion && DisableDynamicTileUrlFormat)
					{
						...
						var url = @"http://www.bing.com/maps";
						var html = GMaps.Instance.UseUrlCache
							? Cache.Instance.GetContent(url, CacheType.UrlCache, TimeSpan.FromDays(7))
							: string.Empty;
						...
						if (!string.IsNullOrEmpty(html))
						{
							...
							if (mat.Success)
							{
								...
								{
									var ver = gc[1].Value;
									var old = GMapProviders.BingMap.Version;
									if (ver != old)
									{
		...
		internal string GetTileUrl(string imageryType)
		{
			...
			if (!string.IsNullOrEmpty(SessionId))
			{
				try
				{
					var url = "http://dev.virtualearth.net/REST/V1/Imagery/Metadata/" + imageryType + "?output=xml&key=" + SessionId;

					var r = GMaps.Instance.UseUrlCache
						? Cache.Instance.GetContent("GetTileUrl" + imageryType, CacheType.UrlCache, TimeSpan.FromDays(7))
						: string.Empty;
					var cache = false;
		...
		{
			var addition = avoidHighways ? "&avoid=highways" : string.Empty;
			var mode = walkingMode ? "Walking" : "Driving";
		...
		private string MakeRouteUrl(PointLatLng start, PointLatLng end, string language, bool avoidHighways, bool walkingMode)
		{
			...
			return string.Format(CultureInfo.InvariantCulture, RouteUrlFormatPointLatLng, mode, start.Lat, start.Lng, end.Lat,
				end.Lng, addition, SessionId);
		}
			...
			out int zoomFactor)
		{
			...
			try
			{
				...
				if (!string.IsNullOrEmpty(route))
				{
					...
					var tooltipEnd = 0;
					{
						var x = route.IndexOf("<RoutePath><Line>") + 17;
						if (x >= 17)
						{
		...
		private static readonly string RouteUrlFormatPointQueries =
	...
	public class BingMapProvider : BingMapProviderBase
	{
		...
		public override void OnInitialized()
		{
			...
			if (!DisableDynamicTileUrlFormat)
			{
				...
				{
					UrlDynamicFormat = UrlDynamicFormat.Replace("{subdomain}", "t{0}")
						.Replace("{quadkey}", "{1}")
						.Replace("{culture}", "{2}");
				}
			}
