	...
	public class GMaps : Singleton<GMaps>
	{
					...
					if (l != null)
					{
						...
						if (t != null)
						{
							t.InvokeMember("Enable",
								BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Static | BindingFlags.InvokeMethod, null, null,
								null);
						}
					}
				}
		...
		public bool ExportToGMDB(string file)
		{
			...
			if (PrimaryCache is SQLitePureImageCache)
			{
				...
				db.AppendFormat(CultureInfo.InvariantCulture, "{0}{1}Data.gmdb", GMapProvider.LanguageStr,
...
#else
					else if (CacheEngine == null || CacheEngine.ThreadState == ThreadState.Stopped ||
					         CacheEngine.ThreadState == ThreadState.Unstarted)
#endif
					{
						CacheEngine = null;
						CacheEngine = new Thread(CacheEngineLoop);
		...
		private volatile bool abortCacheLoop;
		...
		{
			get { return cacheOnIdleRead; }
			set { cacheOnIdleRead = value; }
		}

		private volatile bool boostCacheEngine;
		...
		{
			get { return boostCacheEngine; }
			set { boostCacheEngine = value; }
		}
			...
			while (!abortCacheLoop)
			{
				try
				{
					...
					if (task.HasValue)
					{
						...
						{
							Debug.WriteLine("CacheEngine[" + left + "]: storing tile " + task.Value + ", " + task.Value.Img.Length/1024 +
							                "kB...");
