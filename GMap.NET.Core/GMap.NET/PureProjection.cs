...
namespace GMap.NET
{
	...
	{
		private readonly List<Dictionary<PointLatLng, GPoint>> FromLatLngToPixelCache =
			new List<Dictionary<PointLatLng, GPoint>>(33);

		private readonly List<Dictionary<GPoint, PointLatLng>> FromPixelToLatLngCache =
			new List<Dictionary<GPoint, PointLatLng>>(33);

		public PureProjection()
		{
			for (var i = 0; i < FromLatLngToPixelCache.Capacity; i++)
			{
		...
		public List<GPoint> GetAreaTileList(RectLatLng rect, int zoom, int padding)
		{
			...
			for (var x = (topLeft.X - padding); x <= (rightBottom.X + padding); x++)
			{
				for (var y = (topLeft.Y - padding); y <= (rightBottom.Y + padding); y++)
				{
		...
		{
			var dLat1InRad = p1.Lat*(Math.PI/180);
			var dLong1InRad = p1.Lng*(Math.PI/180);
			var dLat2InRad = p2.Lat*(Math.PI/180);
			var dLong2InRad = p2.Lng*(Math.PI/180);
		...
		public void FromGeodeticToCartesian(double Lat, double Lng, double Height, out double X, out double Y, out double Z)
		{
			...
			var B = Axis*(1.0 - Flattening);
			var ee = 1.0 - (B/Axis)*(B/Axis);
			var N = (Axis/Math.Sqrt(1.0 - ee*Math.Sin(Lat)*Math.Sin(Lat)));
		...
		{
			var E = Flattening*(2.0 - Flattening);
			Lng = Math.Atan2(Y, X);

			var P = Math.Sqrt(X*X + Y*Y);
			var Theta = Math.Atan2(Z, (P*(1.0 - Flattening)));
