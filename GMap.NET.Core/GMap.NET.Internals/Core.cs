   ...
   using Thread=OpenNETCF.Threading.Thread2;
	...
	public class Core : IDisposable
	{
		...
		public GMapProvider Provider
		{
			...
			set
			{
				...
				{
					var diffProjection = (provider == null || provider.Projection != value.Projection);
      ...
      public int LevelsKeepInMemmory = 1;
		...
		public void OnMapSizeChanged(int width, int height)
		{
			...
			if (IsRotated)
			{
				...
				var diag =
					(int)
						Math.Round(Math.Sqrt(Width*Width + Height*Height)/Provider.Projection.TileSize.Width,
	...
	//TODO: clear loading
		...
		private void tileLoadThread()
		{
			...
			var ctid = "Thread[" + ct.ManagedThreadId + "]";
#else
            int ctid = 0;
#endif
				...
				if (!m.NotEmpty)
				{
					...
					foreach (var tl in task.Core.provider.Overlays)
					{
						...
						do
						{
							...
							if (task.Zoom >= task.Core.provider.MinZoom &&
							    (!task.Core.provider.MaxZoom.HasValue || task.Zoom <= task.Core.provider.MaxZoom))
							{
								if (task.Core.skipOverZoom == 0 || task.Zoom <= task.Core.skipOverZoom)
								{
									...
									if (tl.InvertedAxisY)
									{
										img = GMaps.Instance.GetImageFrom(tl, new GPoint(task.Pos.X, task.Core.maxOfTiles.Height - task.Pos.Y),
											task.Zoom, out ex);
									}
									else // ok
									{
							...
							{
								var zoomOffset = task.Zoom > task.Core.okZoom ? task.Zoom - task.Core.okZoom : 1;
								long Ix = 0;
		...
		private void OnLoadComplete(string ctid)
		{
			...
			var lastTileLoadTimeMs = (long) (LastTileLoadEnd - LastTileLoadStart).TotalMilliseconds;
		...
		private void UpdateBounds()
		{
			...
			try
			{
				...
				for (long i = (int) Math.Floor(-sizeOfMapArea.Width*scaleX), countI = (int) Math.Ceiling(sizeOfMapArea.Width*scaleX);
					i <= countI;
					i++)
				{
					for (
						long j = (int) Math.Floor(-sizeOfMapArea.Height*scaleY), countJ = (int) Math.Ceiling(sizeOfMapArea.Height*scaleY);
						j <= countJ;
						j++)
					{
