...
namespace GMap.NET.Projections
{
	...
	public class MapyCZProjection : PureProjection
	{
		...
		private double[] wgsToUTM(double la, double lo, int zone)
		{
			...
			var eterm2 = (Math.Pow(w, 4)/120.0)*Math.Pow(coslat, 4)*
			             (4*Math.Pow(psi, 3)*(1.0 - 6*t*t) + psi*psi*(1.0 + 8*t*t) - psi*2*t*t + Math.Pow(t, 4));
			...
			var nterm3 = (Math.Pow(w, 6)/720.0)*nu*sinlat*Math.Pow(coslat, 5)*
			             (8*Math.Pow(psi, 4)*(11.0 - 24*t*t) - 28*Math.Pow(psi, 3)*(1.0 - 6*t*t) + psi*psi*(1.0 - 32*t*t) -
			              psi*2*t*t + Math.Pow(t, 4));
			var nterm4 = (Math.Pow(w, 8)/40320.0)*nu*sinlat*Math.Pow(coslat, 7)*
			             (1385.0 - 3111*t*t + 543*Math.Pow(t, 4) - Math.Pow(t, 6));
		...
		private double[] utmToWGS(double eastIn, double northIn, int zone)
		{
			...
			var footlat = sigma + ((3*n/2.0) - (27*Math.Pow(n, 3)/32.0))*Math.Sin(2*sigma) +
			              ((21*n*n/16.0) - (55*Math.Pow(n, 4)/32.0))*Math.Sin(4*sigma) +
			              (151*Math.Pow(n, 3)/96.0)*Math.Sin(6*sigma) + (1097*Math.Pow(n, 4)/512.0)*Math.Sin(8*sigma);
			...
			var laterm3 = (t/(k*rho))*(east*Math.Pow(x, 5)/720.0)*
			              (8*Math.Pow(psi, 4)*(11 - 24*t*t) - 12*Math.Pow(psi, 3)*(21.0 - 71*t*t) +
			               15*psi*psi*(15.0 - 98*t*t + 15*Math.Pow(t, 4)) + 180*psi*(5*t*t - 3*Math.Pow(t, 4)) +
			               360*Math.Pow(t, 4));
			var laterm4 = (t/(k*rho))*(east*Math.Pow(x, 7)/40320.0)*
			              (1385.0 + 3633*t*t + 4095*Math.Pow(t, 4) + 1575*Math.Pow(t, 6));
			...
			var loterm3 = (Math.Pow(x, 5)/120.0)*seclat*
			              (-4*Math.Pow(psi, 3)*(1 - 6*t*t) + psi*psi*(9 - 68*t*t) + 72*psi*t*t + 24*Math.Pow(t, 4));
		...
		{
			var wh = zoom > 3 ? (3*(long) Math.Pow(2, zoom - 4)) : 1;
			return new GSize(wh, wh);
		}

		public override GSize GetTileMatrixMaxXY(int zoom)
		{
			var wh = (long) Math.Pow(2, zoom) - (long) Math.Pow(2, zoom - 2);
			return new GSize(wh, wh);
		}
	}
}